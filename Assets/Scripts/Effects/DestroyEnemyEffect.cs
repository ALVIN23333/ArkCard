...
[CardEffect(EffectType.DestoryEnemy, "消灭敌方")]
public sealed class DestroyEnemyEffect : CardEffectDefinitionBase
{
    public override EffectType EffectType => EffectType.DestoryEnemy;
    public override string Label => "消灭敌方";
    ...
    public override void ApplyRuntime(
        ...
        Action onComplete)
    {
        RuntimeEffectActions.DestroyTargets(targets);
        onComplete?.Invoke();
    }
        ...
        Random random)
    {
        ...
        {
            EffectSimulationResolver.KillCard(state, state.FindCard(target.Id), random);
        }
    }
        ...
        SimulatedTarget target)
    {
        ...
        return card == null ? double.MinValue : EffectTargetingRules.GetSimulationThreat(card) + card.Health;
    }
    ...
    public override double ScoreRuntimeTarget(CardController source, CardEffectData effect, UnityEngine.Object target)
    {
        ...
        return EffectTargetingRules.GetRuntimeThreat(card) + card.health;
    }
